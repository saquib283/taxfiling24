...
import { 
  Calculator, AlertCircle, HelpCircle,
  TrendingUp, Home, Briefcase, Wallet, PieChart,
  ShieldCheck, Landmark, Plus, Minus,
...
export default function TaxCalculator({
  ...
  content?: {
    ...
    income?: {
      ...
      netExpensesLabel?: string;
    };
    deductions?: {
      panelTitle?: string;
      panelSubtitle?: string;
      hraGuideLabel?: string;
      hraAssistantTitle?: string;
      basicSalaryLabel?: string;
      hraReceivedLabel?: string;
      rentPaidLabel?: string;
      metroLabel?: string;
      section80cLabel?: string;
      section80cSubtext?: string;
      section80dLabel?: string;
      advancedLabel?: string;
      parents80dLabel?: string;
      npsLabel?: string;
      homeLoanLabel?: string;
      otherDeductionsLabel?: string;
      capitalGainsTitle?: string;
      stcgLabel?: string;
      ltcgLabel?: string;
    };
    compliance?: {
      ...
      panelSubtitle?: string;
      taxPaidLabel?: string;
      delay234ALabel?: string;
      delay234BLabel?: string;
      monthsLabel?: string;
    };
    results?: {
      panelTitle?: string;
  ...
  const incomeCopy = content?.income || {};
  const deductionsCopy = content?.deductions || {};
  const complianceCopy = content?.compliance || {};
  const resultsCopy = content?.results || {};
      ...
      <div className="mx-auto px-4 max-w-3xl">
        <AnimatedSection className="mb-12 text-center">
            <h2 className="mb-3 text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl">
                {heroCopy.titlePrefix || "Tax Calculator"}{" "}
                <span className="text-blue-600">{heroCopy.titleHighlight || "FY 2025-26"}</span>
            </h2>
        ...
        <div className="mb-10 flex flex-col items-center justify-center gap-4">
          <div className="inline-flex h-11 items-center justify-center rounded-xl bg-white p-1 border border-slate-200 shadow-sm">
            ...
            >
              {incomeCopy.citizenLabel || "General"}
            </button>
            <button
            ...
            >
              {incomeCopy.superSeniorLabel || "Super Senior"}
            </button>
          </div>
        </div>
        ...
        <div className="space-y-8">
          ...
          <div className="rounded-[32px] border border-slate-200 bg-white p-8 shadow-sm transition-all hover:shadow-md hover:border-slate-300">
              <div className="mb-8 border-b border-slate-100 pb-5">
                  <h3 className="text-lg font-bold text-slate-900 tracking-tight">{incomeCopy.panelTitle || "Income Sources"}</h3>
                  <p className="text-xs font-bold uppercase tracking-wider text-slate-400 mt-1">{incomeCopy.panelSubtitle || "Primary sources of earnings"}</p>
              </div>

              <div className="space-y-6">
                <InputWrapper label={incomeCopy.salaryLabel || "Base Salary"} icon={<Briefcase className="h-4 w-4" />}>
                  <input
                    type="number"
                    value={salary || ""}
                    placeholder="0"
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <InputWrapper label={incomeCopy.rentalLabel || "Rental Income"} icon={<Home className="h-4 w-4" />}>
                    <input
                      type="number"
                      value={rentalIncome || ""}
                      placeholder="0"
                  ...
                  <InputWrapper label={incomeCopy.otherIncomeLabel || "Other Income"} icon={<HelpCircle className="h-4 w-4" />}>
                    <input
                      type="number"
                      value={otherIncome || ""}
                      placeholder="0"
                ...
                <div className="pt-2">
                    ...
                    >
                      ...
                      {incomeCopy.businessToggleLabel || "Professional / Business Income"}
                    </button>
                    ...
                    <AnimatePresence>
                      {professionalIncome > 0 && (
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mt-4">
                           <div className="grid gap-4 sm:grid-cols-2">
                              <InputWrapper label={incomeCopy.grossReceiptsLabel || "Gross Receipts"} icon={<Users className="h-4 w-4" />}>
                              ...
                              <InputWrapper label={incomeCopy.netExpensesLabel || "Net Expenses"} icon={<PieChart className="h-4 w-4" />}>
          ...
          <div className="rounded-[32px] border border-slate-200 bg-white p-8 shadow-sm transition-all hover:shadow-md hover:border-slate-300">
              <div className="mb-8 border-b border-slate-100 pb-5 flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-bold text-slate-900 tracking-tight">{deductionsCopy.panelTitle || "Tax Deductions"}</h3>
                  <p className="text-xs font-bold uppercase tracking-wider text-slate-400 mt-1">{deductionsCopy.panelSubtitle || "Old Regime Exemptions"}</p>
                </div>
                <button 
                ...
                >
                  ...
                  {deductionsCopy.hraGuideLabel || "HRA Guide"}
                </button>
              </div>
              ...
              <div className="space-y-6">
                  ...
                  {showHraAssistant && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden bg-blue-50/30 rounded-2xl p-6 border border-blue-100/50 mb-6">
                        <div className="flex items-center gap-2 mb-5">
                           ...
                           <p className="text-xs font-bold uppercase text-blue-600 tracking-widest">{deductionsCopy.hraAssistantTitle || "HRA Calculator Assistant"}</p>
                        </div>
                        <div className="grid gap-4 sm:grid-cols-2 mb-4">
                           <InputWrapper label={deductionsCopy.basicSalaryLabel || "Basic Salary"} icon={<Smartphone className="h-3.5 w-3.5" />}><input type="number" value={basicSalary} onChange={e => setBasicSalary(Number(e.target.value))} className="w-full bg-transparent text-sm font-bold text-slate-900 outline-none" /></InputWrapper>
                           <InputWrapper label={deductionsCopy.hraReceivedLabel || "HRA Received"} icon={<Briefcase className="h-3.5 w-3.5" />}><input type="number" value={hraReceived} onChange={e => setHraReceived(Number(e.target.value))} className="w-full bg-transparent text-sm font-bold text-slate-900 outline-none" /></InputWrapper>
                           <InputWrapper label={deductionsCopy.rentPaidLabel || "Rent Paid"} icon={<Home className="h-3.5 w-3.5" />}><input type="number" value={rentPaid} onChange={e => setRentPaid(Number(e.target.value))} className="w-full bg-transparent text-sm font-bold text-slate-900 outline-none" /></InputWrapper>
                           <div className="flex items-center justify-between px-5 bg-white rounded-2xl border border-slate-100 h-[72px]">
                              <span className="text-[10px] font-bold uppercase text-slate-400 tracking-wider">{deductionsCopy.metroLabel || "Metro City?"}</span>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <InputWrapper label={deductionsCopy.section80cLabel || "Sec 80C (Max 1.5L)"} sub={deductionsCopy.section80cSubtext || "PPF, LIC, ELSS..."} icon={<Target className="h-4 w-4" />}>
                  ...
                  <InputWrapper label={deductionsCopy.section80dLabel || "Sec 80D (Self)"} icon={<ShieldCheck className="h-4 w-4" />}>
                ...
                <div className="pt-2">
                    <details className="group border-t border-slate-100">
                        <summary className="list-none py-5 cursor-pointer flex items-center justify-between group">
                           <span className="text-xs font-bold text-slate-500 uppercase tracking-widest group-hover:text-blue-600 transition-colors">{deductionsCopy.advancedLabel || "Advanced Deductions & Equity"}</span>
                        ...
                        <div className="space-y-6 pb-6">
                           <div className="grid gap-4 sm:grid-cols-2">
                              <InputWrapper label={deductionsCopy.parents80dLabel || "80D Parents"} icon={<Users className="h-3.5 w-3.5" />}><input type="number" value={deduction80D_Parents} onChange={e => setDeduction80D_Parents(Number(e.target.value))} className="w-full bg-transparent text-sm font-bold text-slate-900 outline-none" /></InputWrapper>
                              <InputWrapper label={deductionsCopy.npsLabel || "NPS 80CCD(1B)"} icon={<Glasses className="h-3.5 w-3.5" />}><input type="number" value={deductionNPS} onChange={e => setDeductionNPS(Number(e.target.value))} className="w-full bg-transparent text-sm font-bold text-slate-900 outline-none" /></InputWrapper>
                              <InputWrapper label={deductionsCopy.homeLoanLabel || "Home Loan Int."} icon={<Landmark className="h-3.5 w-3.5" />}><input type="number" value={homeLoanInterest} onChange={e => setHomeLoanInterest(Number(e.target.value))} className="w-full bg-transparent text-sm font-bold text-slate-900 outline-none" /></InputWrapper>
                              <InputWrapper label={deductionsCopy.otherDeductionsLabel || "Other 80G/E..."} icon={<HelpCircle className="h-3.5 w-3.5" />}><input type="number" value={otherDeductions} onChange={e => setOtherDeductions(Number(e.target.value))} className="w-full bg-transparent text-sm font-bold text-slate-900 outline-none" /></InputWrapper>
                           </div>
                           
                           <div className="rounded-xl border border-slate-100 bg-slate-50 p-5 mt-6">
                              <p className="text-[9px] font-bold uppercase text-slate-400 mb-4 tracking-widest flex items-center gap-2 rounded-lg"><TrendingUp className="h-3.5 w-3.5" /> {deductionsCopy.capitalGainsTitle || "Capital Gains (Equity Only)"}</p>
                              <div className="grid gap-4 sm:grid-cols-2">
                                 <InputWrapper label={deductionsCopy.stcgLabel || "STCG (Short Term)"} icon={<Briefcase className="h-3.5 w-3.5" />}><input type="number" value={stcgEquity} onChange={e => setStcgEquity(Number(e.target.value))} className="w-full bg-transparent text-sm font-bold text-slate-900 outline-none" /></InputWrapper>
                                 <InputWrapper label={deductionsCopy.ltcgLabel || "LTCG (Long Term)"} icon={<Wallet className="h-3.5 w-3.5" />}><input type="number" value={ltcgEquity} onChange={e => setLtcgEquity(Number(e.target.value))} className="w-full bg-transparent text-sm font-bold text-slate-900 outline-none" /></InputWrapper>
                              </div>
                           </div>
                        </div>
                    </details>
          ...
          <div className="rounded-[32px] border border-slate-200 bg-white p-8 shadow-sm transition-all hover:shadow-md hover:border-slate-300">
             <div className="mb-8 border-b border-slate-100 pb-5">
                <h3 className="text-lg font-bold text-slate-900 tracking-tight">{complianceCopy.panelTitle || "Payments & Compliance"}</h3>
                <p className="text-xs font-bold uppercase tracking-wider text-slate-400 mt-1">{complianceCopy.panelSubtitle || "TDS, Advance tax, and delay interests"}</p>
             </div>
             <div className="grid gap-4 sm:grid-cols-2">
                <InputWrapper label={complianceCopy.taxPaidLabel || "TDS / Advance Tax Paid"} icon={<Target className="h-4 w-4" />}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <InputWrapper label={complianceCopy.delay234ALabel || "234A Delay"} sub={complianceCopy.monthsLabel || "Months"} icon={<AlertCircle className="h-3.5 w-3.5" />}><input type="number" value={monthsDelay234A} onChange={e => setMonthsDelay234A(Number(e.target.value))} className="w-full bg-transparent text-sm font-bold text-slate-900 outline-none" /></InputWrapper>
                  <InputWrapper label={complianceCopy.delay234BLabel || "234B Delay"} sub={complianceCopy.monthsLabel || "Months"} icon={<AlertCircle className="h-3.5 w-3.5" />}><input type="number" value={monthsDelay234B} onChange={e => setMonthsDelay234B(Number(e.target.value))} className="w-full bg-transparent text-sm font-bold text-slate-900 outline-none" /></InputWrapper>
                </div>
             </div>
          </div>
          ...
          <div className="overflow-hidden rounded-2xl border border-slate-900 bg-slate-900 text-white shadow-xl">
              <div className="p-8 pb-10">
                <div className="mb-8 flex items-center justify-between border-b border-white/10 pb-6">
                    <div>
                        <h3 className="text-lg font-bold tracking-tight">{resultsCopy.panelTitle || "Tax Report"}</h3>
                ...
                <div className="space-y-8">
                  <div className="flex flex-col items-center justify-center py-6 text-center">
                    <p className="text-xs font-bold uppercase tracking-[0.2em] text-white/30 mb-4">{resultsCopy.recommendedLabel || "Recommended Strategy"}</p>
                    <div className="rounded-full bg-gradient-to-r from-blue-600/20 to-indigo-600/20 px-6 py-2.5 text-xs font-bold uppercase tracking-widest text-blue-400 border border-blue-500/30 shadow-lg shadow-blue-500/10">
                        {results.better} {resultsCopy.strategySuffix || "SAVES YOU MOST"}
                    </div>
                    <p className="mt-8 text-6xl font-black tracking-tighter text-white">{formatCurrency(results.savings)}</p>
                    <p className="mt-2 text-[11px] font-bold uppercase tracking-widest text-white/40">{resultsCopy.savingsLabel || "Total Net Savings Estimate"}</p>
                  </div>

                  <div className="grid grid-cols-2 gap-4 border-y border-white/5 py-8">
                    <div className="text-center">
                        <p className="text-[9px] font-bold uppercase tracking-widest text-white/20 mb-1">{resultsCopy.oldRegimeLabel || "Old Regime"}</p>
                        <p className="text-xl font-bold text-white/60">{formatCurrency(results.oldTax)}</p>
                    </div>
                    <div className="text-center border-l border-white/5">
                        <p className="text-[9px] font-bold uppercase tracking-widest text-white/20 mb-1">{resultsCopy.newRegimeLabel || "New Regime"}</p>
                  ...
                  <div className="space-y-4 px-4 pt-4">
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] font-bold uppercase tracking-widest text-white/30">{resultsCopy.taxableIncomeLabel || "Taxable Income"}</span>
                    ...
                    {(results.interest234A > 0 || results.interest234B > 0) && (
                       <div className="flex justify-between items-center border-y border-white/5 py-3">
                          <span className="text-[10px] font-bold uppercase tracking-widest text-red-400">{resultsCopy.delayInterestLabel || "Delay Interest (234A/B)"}</span>
                  ...
                  {results.totalPayable !== (results.better === "New Regime" ? results.newTax : results.oldTax) && (
                     <div className="mt-6 rounded-xl bg-white/5 p-5 border border-white/10">
                        <div className="flex justify-between items-center">
                           <span className="text-sm font-bold uppercase tracking-widest text-white/50">{resultsCopy.totalPayableLabel || "Total Final Payable"}</span>
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-4">
                    ...
                    >
                      <Download className="h-4 w-4" /> {resultsCopy.downloadLabel || "Download Statement"}
                    </button>
                    <button 
                    ...
                    >
                      <Printer className="h-4 w-4" /> {resultsCopy.printLabel || "Print Summary"}
                    </button>
                  </div>
                </div>
              </div>
