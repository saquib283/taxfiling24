export const CONTACT = {
  phone: "+91 7011246157",
  phoneRaw: "917011246157",
  email: "support@taxfiling24.com",
  address: "E-244/G First Floor Shaheen Bagh, Okhla New Delhi 110025",
  whatsapp: "https://wa.me/917011246157",
};

export const SERVICES = [
  {
    title: "BUSINESS SETUP & REGISTRATION SERVICES",
    description: "Company registration, LLP, Partnership, and more",
    href: "/services/business-setup-registration-services",
  },
  {
    title: "LICENCES, IP, CERTIFICATIONS & LEGAL REGISTRATIONS",
    description: "Trademarks, licences, certifications",
    href: "/services/licences-ip-certifications-legal-registrations",
  },
  {
    title: "TAXATION, GST & COMPLIANCE MANAGEMENT",
    description: "GST filing, tax compliance, and management",
    href: "/services/taxation-gst-compliance-management",
  },
  {
    title: "CORPORATE, ROC & SECRETARIAL FILINGS",
    description: "Annual filings, ROC compliance",
    href: "/services/corporate-roc-secretarial-filings",
  },
  {
    title: "ACCOUNTING, FINANCIAL MANAGEMENT & REPORTING",
    description: "Bookkeeping, financial reporting",
    href: "/services/accounting-financial-management-reporting",
  },
  {
    title: "AUDIT, FORENSIC, RISK & CORPORATE INVESTIGATION",
    description: "Audits and investigations",
    href: "/services/audit-forensic-risk-corporate-investigation",
  },
  {
    title: "SPECIAL SERVICES, E-COMMERCE, BANKING & DOCUMENTATION",
    description: "E-commerce and banking support",
    href: "/services/special-services-e-commerce-banking-documentation",
  },
];

export const FEATURES = [
  {
    title: "100% Compliance Guarantee",
    description:
      "We ensure your business stays fully compliant with all regulatory requirements. Zero penalties, zero stress.",
  },
  {
    title: "Expert & Certified Team",
    description:
      "Certified professionals with extensive experience in taxation and compliance",
  },
  {
    title: "Lightning Fast Turnaround",
    description:
      "Quick processing times without compromising quality. Most registrations completed within 7-15 days.",
  },
  {
    title: "Affordable & Transparent Pricing",
    description:
      "Transparent pricing with no hidden costs, ensuring you always know exactly how much you're investing before we begin.",
  },
  {
    title: "Tax & Compliance Reminders",
    description:
      "Never miss a deadline with our proactive reminder system for all your recurring compliances and filings.",
  },
  {
    title: "24/7 Quick Customer Support",
    description:
      "Round-the-clock support via phone, email, and WhatsApp. We're here whenever you need us.",
  },
];

export const ABOUT_FEATURES = [
  {
    title: "Accuracy",
    description: "100% Error-free filings and Documentation",
  },
  {
    title: "Expertise",
    description: "Certified Teams + deep industry knowledge",
  },
  {
    title: "Affordability",
    description: "Transparent pricing with no hidden costs",
  },
  {
    title: "Client - First",
    description: "Personalized support tailored to your needs",
  },
];

export const TESTIMONIALS = [
  {
    name: "Mehboob Afzal",
    company: "Afzal Healthcare",
    text: "Taxfiling24 made our company registration incredibly smooth. They handled everything from naming to DSC procurement in less than two weeks!",
    rating: 5,
  },
  {
    name: "Shabnam Reyaz",
    company: "Shina Decorative Bags",
    text: "We used them for GST compliance. Their proactive reminders saved us from late fees completely. Highly professional and transparent.",
    rating: 5,
  },
  {
    name: "Mehtab Alam",
    company: "Exotic Furnishers",
    text: "Their Virtual CFO services provided our SME with high-end financial strategy without the overhead of a full-time executive. Game changer.",
    rating: 5,
  },
  {
    name: "Amit Kumar",
    company: "Kumar Enterprises",
    text: "Managing corporate tax and ROC filings used to be stressful. Taxfiling24’s expert team ensured zero penalties and smooth annual reporting.",
    rating: 5,
  },
];

export const FAQ_ITEMS = [
  {
    question: "How long does it take to register a Private Limited Company?",
    answer:
      "Typically, the entire process takes 10-15 working days from document submission to incorporation certificate. This includes name approval, DSC procurement, filing of SPICe forms, and final incorporation. Timeline may vary based on government processing and document accuracy.",
  },
  {
    question: "What documents are required for GST registration?",
    answer:
      "You'll need: PAN Card, Aadhaar Card, Business registration proof (Partnership Deed/MOA/AOA), Address proof of business premises, Bank account statement/cancelled cheque, Photograph of proprietor/partners/directors, and authorization letter (if applicable). We'll guide you through the entire documentation process.",
  },
  {
    question: "Do I need to file ITR if my income is below taxable limit?",
    answer:
      "While not mandatory, filing ITR even below taxable limit is recommended. It serves as income proof for loans, visa applications, and government tenders. It also helps claim refunds if TDS was deducted and builds a financial track record. We offer affordable ITR filing services for all income levels.",
  },
  {
    question:
      "What are the annual compliance requirements for a Pvt Ltd Company?",
    answer:
      "Key annual compliances include: Filing Annual Returns (MGT-7), Financial Statements (AOC-4), Income Tax Returns, conducting Board Meetings (minimum 4 per year), Annual General Meeting (AGM), maintaining statutory registers, and Director KYC (DIN eKYC). We provide comprehensive annual compliance packages to ensure you never miss a deadline.",
  },
  {
    question: "How much does trademark registration cost?",
    answer:
      "Trademark registration costs vary based on type (individual/startup/company) and number of classes. Government fees range from ₹4,500-9,000 per class. Our professional fees are transparent and affordable. The entire process takes 12-18 months for complete registration. Contact us for exact pricing based on your requirements.",
  },
  {
    question: "Can you help with Income Tax notices and assessments?",
    answer:
      "Yes! Our experienced CAs specialize in tax litigation. We handle all types of IT notices (scrutiny, demand, defective return), prepare and file responses, represent you before tax authorities, assist in appeals, and work to minimize tax liability legally. Early consultation helps achieve better outcomes.",
  },
  {
    question: "What is Virtual CFO service and who needs it?",
    answer:
      "Virtual CFO provides strategic financial management without hiring a full-time CFO. It's ideal for startups and SMEs who need financial planning, budgeting, cash flow management, MIS reports, fundraising support, and business advisory. You get expert guidance at a fraction of the cost of a full-time CFO.",
  },
  {
    question:
      "Do you provide services across India or only in specific cities?",
    answer:
      "We provide services PAN India! With digital processes and online consultations, we serve clients from all states and cities. Whether you're in metro cities or tier-2/3 towns, you get the same quality of service. Most documentation can be handled remotely, with physical presence required only in rare cases.",
  },
];

export const PRICING_PLANS = [
  {
    name: "Basic",
    price: "₹4,999",
    description: "For startups and small businesses",
    features: ["GST Registration", "Basic ITR Filing", "Email Support"],
    highlighted: false,
  },
  {
    name: "Standard",
    price: "₹9,999",
    description: "For growing businesses",
    features: [
      "All Basic features",
      "Company Registration",
      "GST + ITR Filing",
      "24/7 Phone & WhatsApp Support",
    ],
    highlighted: true,
  },
  {
    name: "Premium",
    price: "₹19,999",
    description: "For established businesses",
    features: [
      "All Standard features",
      "Full Compliance Package",
      "Dedicated CA Support",
      "Priority Processing",
    ],
    highlighted: false,
  },
];

export const FORM_SERVICES = [
  "BUSINESS SETUP & REGISTRATION SERVICES",
  "LICENCES, IP, CERTIFICATIONS & LEGAL REGISTRATIONS",
  "TAXATION, GST & COMPLIANCE MANAGEMENT",
  "CORPORATE, ROC & SECRETARIAL FILINGS",
  "ACCOUNTING, FINANCIAL MANAGEMENT & REPORTING",
  "AUDIT, FORENSIC, RISK & CORPORATE INVESTIGATION",
  "SPECIAL SERVICES, E-COMMERCE, BANKING & DOCUMENTATION",
  "Other",
];
